...
# --- CORE LOGIC (Modified to return bytes for Streamlit) ---
...
def generate_all_call_data(phone_numbers, start_date, end_date, business_start, business_end, total_calls_per_day, avg_answered_total, jitter_max):
    ...
    while current_date <= end_datetime:
        ...
        for phone in phone_numbers:
            ...
            attempt = 1
            daily_statuses = ["Answered"] * avg_answered_total
            for _ in range(remaining_calls_count):
            ...
            for call_status in daily_statuses:
                ...
                all_records.append({
                    "Date Time": time_cursor.strftime("%d-%m-%Y %H:%M"),
                    "Attempt": attempt,
                    "Lead ID": lead_id,
                    "Status": call_status,
                    "Length (s)": length,
...
def create_pdf_bytes(df):
    ...
    for row in df.values.tolist():
        pdf.set_text_color(0, 100, 0) if row[3] == "Answered" else pdf.set_text_color(0, 0, 0)
        print_row(row)
...
with st.sidebar:
    ...
    date_range = st.date_input("Date Range", [datetime(2025, 11, 1), datetime(2025, 11, 5)])
